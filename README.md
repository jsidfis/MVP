...
当前状态是 MVP 便携体验版：可以本地运行核心流程，也可以生成 zip 发给其他用户试用。Phase 6 已补充开源发布准备；正式发布前的今日工作台 UI 重构也已完成。

## 功能范围

已包含：

- 分阶段今日工作台：计划全宽录入、执行全屏视图、复盘抽屉
- 文件夹视图和星系视图，并记住上次选择
- 主页视图切换，并记住上次选择
- 四象限任务管理
...
- `docs/PROJECT_UI_REFRESH.md`：正式发布前的今日工作台界面重构说明。
...
- `计划`：使用分步卡片新增任务；任务模板按需展开。模板只保存标题、四象限和计划耗时字段，不保存执行状态、实际耗时、顺延原因或复盘内容。
- `重复`：新增任务时可以选择 `不重复`、`每天`、`工作日` 或 `每周`。重复任务不会后台运行；应用打开某一天时，会检查本地规则并生成当天应出现的任务。同一规则同一天只生成一次。
- `执行中`：文件夹或星系视图占满主要窗口；底部工具坞按需打开新增任务、今日任务、搜索和设置。
- `复盘`：保留降亮后的当日视图，通过右侧抽屉处理未完成任务并逐项回答四问。
- `本地搜索`：在执行工具坞中按关键词、日期、四象限、状态和顺延原因筛选本地历史任务。搜索只读取本地 SQLite 数据，不联网、不上传。
